...
- [2025-11-02] On `feature/fix-dashboard-onaction`: fixed Bootstrap failure when wiring Dashboard buttons by qualifying `OnAction` with the workbook name and falling back to `Shape.OnAction` if needed. Keeps button behavior the same and avoids the "Unable to set the OnAction property of the Button class" error during `SeedNamedValues`.
 ...
 - [2025-11-02] On `feature/beautify-bug-wip-sections`: fixed compile error (undeclared `row2`); styled Bug Metrics (Sprint/Quarter) as tables with section headers and framed panels; normalized arrows/dashes in labels; avoided duplicate WIP sections when building from WIP-like sources already processed into Jira_Facts.
 - [2025-11-02] On `feature/beautify-bug-wip-sections`: updated Jira_Insights to call `Flow_AppendChartsToSheet_EX` (renamed to avoid VBE cache ambiguity), and aligned Sanitize path call; confirmed no colon-chained `On Error` remain.
 - [2025-11-02] Temporary flags added: `FLOW_ENABLE_WIP=0` and `FLOW_ENABLE_SCATTER=0` to disable WIP Aging and Cycle Scatter while stabilizing. Throughput and Sprint Spans remain enabled.
 - [2025-11-13] On `feature/fix-sanitize-source-sheet-resolution`: re-enabled `Flow_AppendChartsToSheet_EX` with a minimal, compile-safe implementation that appends Sprint Spans and Throughput only; WIP Aging and Cycle Scatter remain behind runtime flags. Restored append path from `Jira_CreatePivotsAndCharts` and `SanitizeRawAndBuildInsights`.
 - [2025-11-13] On `feature/epic-burndown-topN`: added all-epics burndown writer `Jira_WriteEpicBurndown_All` and integrated it into `Jira_CreatePivotsAndCharts`. Appends a "Epic Burndown (top N)" multi-series line chart to `Jira_Insights` (top epics by total StoryPoints; unresolved extend to today; range clamped to last 90 days when span >120). Minimal formatting, framed panel.
- [2025-11-13] On `feature/update-sprint-pattern`: changed the default `SprintNamePattern`/`SprintParsePattern` to `Q{Q}.S{S}.{YY}.{TEAM}`, updated the module defaults, and documented the new format.
